...
Latest release: **2.15.0** (2026-09-04).
...
- Optimized history cache write and memory reuse: compact JSON serialization, dirty check to avoid redundant disk writes, and object reuse on cache hits.
- Added Shiki syntax highlighting LRU cache in chat timeline and file change history webviews, eliminating redundant high-cost re-highlighting during rerenders and folding toggles.
- Tree views (History, Pinned, Search) now implement `resolveTreeItem` for deferred tooltip generation on hover, dramatically speeding up tree node expansion.
...
- Reduced history rebuild/refresh file I/O by collecting session meta, last-activity, and leading preview messages in a single read pass.
- Session analysis reuses the timeline scan instead of re-reading the session file, cutting redundant scans per analyzed session.

## What's New in 2.13.0

- Added Simplified Chinese localization for the interface (`ui.language = zh`), including translated menus, commands, submenus, and webviews, with tri-state language switching (`ja` / `zh` / `en`).
- Added a `check:zh-sync` script so future upstream additions can be kept synchronized with the Chinese localization.
- Fixed the Settings page rendering blank when the UI language is set to Chinese.
